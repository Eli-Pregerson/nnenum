#!/bin/bash
# run measurements for all categories using instances_conv.csv files
# categories without instances_conv.csv are silently skipped
...
#
# for example ./run_conv_categories.sh v1 ~/nnenum/vnncomp_scripts . ./out.csv ./counterexamples "test acasxu" all
...
do
    INSTANCES_CSV_PATH="${VNNCOMP_FOLDER}/benchmarks/${CATEGORY}/instances_conv.csv"
    ...
    then
	    echo "$INSTANCES_CSV_PATH file not found, skipping category $CATEGORY"
	    IFS=$PREV_IFS
	    continue
    fi
